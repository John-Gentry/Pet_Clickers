...
function randomdirection(Egg)
    ...
    if number > 70 then
        x=TweenService:Create(Egg,Up,{CFrame = CFrame.new(156, math.random(40,45), -400)*CFrame.Angles(math.random(5,20), math.random(5,20), math.random(5,20))})
        x:Play()
        x.Completed:Wait()
        x=TweenService:Create(Egg,Down,{CFrame = CFrame.new(156, 37.54, -400)})
        x:Play()
        x.Completed:Wait()
    else
        ...
        x:Play()
        x.Completed:Wait()
        x=TweenService:Create(Egg,DownRotate,{CFrame = CFrame.new(156, 37.54, -400)})
        x:Play()
        x.Completed:Wait()
    end
end
